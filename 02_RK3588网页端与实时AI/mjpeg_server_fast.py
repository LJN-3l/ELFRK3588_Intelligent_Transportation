...
class FrameHub:
    def __init__(self) -> None:
        ...
        self.reader_started_at = ""

    def set_reader_state(self, online: bool, error: str = "") -> None:
        with self.cond:
            self.reader_online = online
            if online and not self.reader_started_at:
                self.reader_started_at = now_text()
            if error:
    ...
    def mark_invalid_header(self, error: str) -> None:
        with self.cond:
            self.invalid_headers += 1
            self.last_error = error

    def mark_invalid_jpeg(self, error: str) -> None:
        with self.cond:
            self.invalid_jpegs += 1
            self.last_error = error

    def wait_for_frame(self, previous_id: int, timeout: float = 15.0) -> Tuple[int, bytes]:
